...
let profilesTable: Airtable.Table<FieldSet> | undefined;
...
export interface Profile {
  ...
  status: string;
  youtube_link?: string;
  resume?: unknown | null;  // safer than any
}
...
export async function getActiveProfiles(): Promise<Profile[]> {
  ...
  try {
    const records = await profilesTable.select({ filterByFormula: "{status} = 'Active'" }).firstPage();
    ...
    return records.map(record => ({
      ...
      skills: Array.isArray(record.fields.skills) ? (record.fields.skills as string[]) : [],
      ...
      resume: record.fields.resume ?? null,
    }));
  } catch (error) {
